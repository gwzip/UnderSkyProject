    ...
    public class Weapon : MonoBehaviour
    {
        ...
        public void Holster() // Always opposite to armed
        {
            ...
            else
            {
                ...
                if (type == Type.Sword) transform.localRotation = Quaternion.Euler(Vector3.forward * -90); // 90 degree offset to sword
        ...
        private void OnTriggerStay2D(Collider2D collision)
        {
            ...
            if (collision.TryGetComponent(out Entity entity) || collision.GetComponent<TargetDummy>())
            {
                if (wielder != entity)
                {
                    ...
                    else if (entity is BossNecromancer)
                    {
                        if (wielder == Player.instance) entity.GetComponent<BossNecromancer>().TakeDamage(damageTaken); // Take damage as usual
                        else return; // return so you don't get locked in swordInvincible state
                    }
